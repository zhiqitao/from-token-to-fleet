...
individual host throughputes; it is modulated by scheduling overhead,
network contention, and the cost of moving data between hosts and
storage. In a well-designed fleet, the marginal cost per additional
query declines as the fleet grows, because fixed costs (model loading,
infrastructure) are amortized across more concurrent requests.
...
\textasciitilde2,000 users, \textasciitilde5\% concurrent, generating
\textasciitilde10 rps average and \textasciitilde40 rps peak. With
\textasciitilde9,200 in-tokens and \textasciitilde300 out-tokens per
query, and a 70B dense FP16 model running on 8x H100 (640 GB total
HBM3), the fleet must sustain roughly 40 concurrent queries at peak,
each requiring a full forward pass through the model.

\section{Worked Example}\label{worked-example}

Consider a fleet of N hosts, each with 8× H100 running a single 70B FP16
...
\textbf{Latency floor first.} A 300-output-token request at a
\textasciitilde25 ms/token decode rate (\hyperref[chap:8]{Chapter 8}) takes
\textasciitilde7.5 s of generation alone, before prefill and tool calls.
So a per-query ``16 ms median latency'' is not a real served-request
number --- it would only describe a trivially short completion. The
honest framing is \emph{throughput capacity}, not countless instant
requests: a host that sustains \textasciitilde35,000 tokens/s (achieved,
\hyperref[chap:15]{Chapter 15}) with \textasciitilde16 concurrent full-context requests (KV
ceiling, \hyperref[chap:17]{Chapter 17}) serves roughly 3.7 requests/s at steady state ---
not 250. The ``250 QPS, 16 ms'' figures in this chapter are an
{[}ILLUSTRATIVE{]} exercise in fleet arithmetic, not a measurable
property of this workload; the architect must replace them with the
throughput and KV numbers measured on the actual model+context.
...
At the canonical peak of 40 rps, a fleet sized by KV residency (\hyperref[chap:17]{Chapter 17}, \textasciitilde4--5 hosts for 40 concurrent at T=0) already holds
the peak; the earlier ``2,000 QPS at 2\% utilization'' framing is
misleading because it inflates per-host capacity with an impossible
latency. The correct statement is: the fleet must satisfy both RPS and
KV-residency, whichever binds first.
...
\textbf{Autoscaling economics.} Each H100 host costs
\textasciitilde\$3/hour (cloud) or \textasciitilde\$1.50/hour amortized
(owned) --- {[}ILLUSTRATIVE{]}. The marginal cost per additional QPS is
\$3 ÷ (QPS gained per host); the economic lesson (diminishing cost of a
marginal host once fixed costs dominate) is sound, but the exact per-QPS
figure is only meaningful after the real per-host throughput is
benchmarked.
...
\textbf{Median latency} & 50th-percentile end-to-end query latency &
\textless{} 30 ms \\
\textbf{99th-percentile latency} & Tail latency across the fleet &
\textless{} 100 ms \\
...
\item
  ...
  or re-warmed after idle, model loading takes 30--120 seconds.
  Autoscaling policies must account for this warm-up window; otherwise,
  the fleet experiences a spike in error rate or timeout during
  scale-out events.
\end{itemize}
...
query, 70B dense FP16 model, 8x H100 per host, 8 hosts total.

\textbf{Question}: What is the fleet's aggregate QPS capacity at batch
size 4, and what is the marginal cost per QPS when scaling from 8 to 16
hosts? What scheduling overhead must be kept below to ensure the
99th-percentile latency stays under 100 ms?

\textbf{Solution} (illustrative algebra --- the per-host figure below is
an {[}ILLUSTRATIVE{]} hypothetical for showing the fleet math; the real
...
\tightlist
\item
  \textbf{Real per-host throughput (Ch. 15/17 basis):} a host sustaining
  \textasciitilde35,000 tokens/s (achieved, \hyperref[chap:15]{Chapter 15}) against a
  \textasciitilde9,500-token request moves \textasciitilde3.7 requests/s
  at steady state. This is the number to use for capacity and cost: it
  is grounded in the decode-rate and KV-residency analysis, not an
  assumed QPS.
\item
  Fleet aggregate (8 hosts): 8 × 3.7 ≈ \textbf{30 req/s},
  i.e.~comfortably above the canonical 40 rps peak only when concurrency
  is also held --- the binding constraint is KV residency (\hyperref[chap:17]{Chapter 17}),
  which sets \emph{concurrent} requests, not the QPS arithmetic above.
\item
  \textbf{Marginal cost (real basis):} adding one host at
  \textasciitilde\$3/hr (cloud, {[}ILLUSTRATIVE{]}) buys
  \textasciitilde3.7 req/s, so marginal cost ≈ \$3 ÷ 3.7 ≈
  \textbf{\$0.81 per req/s per hour.} (The earlier illustrative \$0.012
  figure assumed 250 req/s per host --- a 68× understatement that comes
  entirely from the impossible latency; with a real multi-second decode
  per request the per-host rate is \textasciitilde3.7, not 250.)
\item
  ...
  throughput from \hyperref[chap:14]{Chapter 14}, or the \textasciitilde3.7 req/s bound
  derived here.
\item
  Scheduling overhead must stay below \textasciitilde5\% of per-query
  time; against a real decode of seconds this is loose, but measured
  latency is the true anchor (.5 ms dispatch is negligible next to any
  realistic per-request time).
\end{itemize}

\textbf{\hyperref[tab:20.1]{Table 20-1}} --- Fleet QPS and latency vs.~host count, batch
size, and scheduling overhead.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\hyperref[fig:20.1]{Fig 20.1} - Fleet QPS and latency vs.~host count, batch size, and scheduling overhead {[}2° DERIVED{]}},width=\maxwidth{\textwidth},keepaspectratio]{figures/fig-20-2001.pdf}}
\caption{\hyperref[fig:20.1]{Fig 20.1} - Fleet QPS and latency vs.~host count, batch size,
and scheduling overhead {[}2° DERIVED{]}}
